...
function App() {
  ...
  const [generationSource, setGenerationSource] = useState<"live" | "simulation">("simulation");
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      const generated = await generateSandboxResponse(customPrompt || "Hello", liveMode, (prompt) => api.generate(prompt));
      if (cancelled) return;
      setCustomResponse(generated.response);
      setCustomTelemetry(generated.telemetry);
      setGenerationSource(generated.source);
      setIsGeneratingResponse(false);
    }, 300);
    return () => { cancelled = true; window.clearTimeout(timer); };
  }, [customPrompt, isCustom, liveMode]);
      ...
      <section className="workspace" id="evaluation-console">
        ...
        <div className="panel panel--scenario">
          ...
          {isCustom ? <div className="sandbox-grid">
            ...
            <label className="live-mode-toggle"><input type="checkbox" checked={liveMode} onChange={(event) => { setLiveMode(event.target.checked); setResult(null); }} /><span>Try live Gemini <em>(experimental)</em></span><small>Off by default for a fast, reliable demo.</small></label>
            <label>Generated AI response<textarea value={customResponse} readOnly aria-readonly="true" maxLength={8000} /></label>
            <div className="sandbox-draft-note"><span>{isGeneratingResponse ? "Generating" : generationSource === "live" ? "Live (Gemini 3.5 Flash-Lite)" : "Simulated (demo mode)"}</span><p>{generationSource === "live" ? "Live output is held for ControlPlane’s policy checks before any release decision." : "Local demo output is ready immediately and follows the same policy-check flow."}</p></div>
