...
function App() {
  ...
  const [generationSource, setGenerationSource] = useState<"gemini" | "simulation">("simulation");
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        const generated = await api.generate(customPrompt || "Hello");
        if (cancelled) return;
        setCustomResponse(generated.response);
        setCustomTelemetry({ latency_ms: generated.latency_ms, token_count: generated.token_count, retry_count: generated.retry_count });
        ...
        setGenerationSource("simulation");
      } finally {
        if (!cancelled) setIsGeneratingResponse(false);
      }
    }, 300);
      ...
      <section className="workspace" id="evaluation-console">
        ...
        <div className="panel panel--scenario">
          ...
          {isCustom ? <div className="sandbox-grid">
            ...
            <div className="sandbox-draft-note"><span>{isGeneratingResponse ? "Generating" : generationSource === "gemini" ? "Gemini" : "Demo simulation"}</span><p>{generationSource === "gemini" ? "Generated by Gemini, then held for ControlPlane’s policy checks before any release decision." : "Gemini is not configured or unavailable, so this uses a local demo simulation."}</p></div>
